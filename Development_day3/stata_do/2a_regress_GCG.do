set more off
clear
cd "C:\Users\endmu284\Documents\Size\Data\S022_GCG\"
...
replace conditions = "gcg_crispants" if condition == 1
...
preserve
regress `outcome' i.condition tod i.batch
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS\S022_GCG`outcome'_est", replace)
use "RESULTS\S022_GCG`outcome'_est", clear
gen N=e(N)
save "RESULTS\S022_GCG`outcome'_est.dta", replace
restore
}
...
* now append
clear
save "RESULTS\S022_GCG.dta", replace emptyok 

foreach file in intlength_est  ///
intangle_est 
{

use "RESULTS\S022_GCG`file'", clear
gen model="`file'"
append using "RESULTS\S022_GCG.dta"
save "RESULTS\S022_GCG.dta", replace
}
use "RESULTS\S022_GCG.dta", clear
...
replace parm = "gcg_crispants" if parm == "1.condition"
...
save "RESULTS\S022_GCG_results.dta", replace
outsheet using "RESULTS\S022_GCG_results.txt", noquote replace
outsheet using "C:\Users\endmu284\Documents\Size\Data\RESULTS\S022_GCG_results.txt", noquote replace

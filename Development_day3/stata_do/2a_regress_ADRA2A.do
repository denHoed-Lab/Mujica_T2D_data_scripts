set more off
clear
cd "C:\Users\endmu284\Documents\Size\Data\S018_ADRA2A\"
...
replace conditions = "adra2a_crispants" if condition == 1
...
foreach outcome in intlength  intangle ///
 {


preserve
regress `outcome' i.condition tod
parmest, format(estimate %8.6f min95 max95 %8.6f p %8.1e) saving ("RESULTS\S018_ADRA2A`outcome'_est", replace)
use "RESULTS\S018_ADRA2A`outcome'_est", clear
gen N=e(N)
save "RESULTS\S018_ADRA2A`outcome'_est.dta", replace
restore
}
...
* now append
clear
save "RESULTS\S018_ADRA2A.dta", replace emptyok 

foreach file in intlength_est  intangle
{

use "RESULTS\S018_ADRA2A`file'", clear
gen model="`file'"
append using "RESULTS\S018_ADRA2A.dta"
save "RESULTS\S018_ADRA2A.dta", replace
}
use "RESULTS\S018_ADRA2A.dta", clear
...
replace parm = "adra2a_crispants" if parm == "1.condition"
...
* save sum stats and for figures
save "RESULTS\S018_ADRA2A_results.dta", replace
outsheet using "RESULTS\S018_ADRA2A_results.txt", noquote replace
outsheet using "C:\Users\endmu284\Documents\Size\Data\RESULTS\S018_ADRA2A_results.txt", noquote replace
